...
SWITCH_BETWEEN_PROPORTIONAL_AND_MONOSPACE = 0
...
class Terminal:
    def __init__(self,width=80,height=39):
        self.width=width
        self.height=height
        self.init()
        self.reset()
...
#        if self.buf=='': print "ESC %r\n"%e
    ...
    def dumphtml(self,color=1):
        ...
        for i in range(h*w):
            ...
            if i%w==w-1:
                span+='\n'
        r='<?xml version="1.0" encoding="ISO-8859-1"?><pre class="term" id="termpre">%s</pre>'%r
        r = r.replace("_", "<span class='underscore'>_</span>")
        #r = r.replace(".", "<span class='period'>.</span>")
        #r = r.replace("&nbsp;", "&emsp;")
        #r = r.replace(" ", "  ")
        #r = r.replace("\xa0", "&emsp;")
        ##result = []
        ##in_tag = 0
        ##for current in r:
            ##if current == "<":
            ...
            ##if current == ">":
                ##in_tag = 0
            ##if not in_tag and current == "\xa0":
                ##result.append("&emsp;")
            ##else:
                ##result.append(current)
        ##r = "".join(result)
        if not SWITCH_BETWEEN_PROPORTIONAL_AND_MONOSPACE:
            r = r.replace("\xa0", "\xa0\xa0")
        print r
        if self.last_html==r:
